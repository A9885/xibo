...
class AccessTokenRepository implements AccessTokenRepositoryInterface
{
    ...
    public function persistNewAccessToken(AccessTokenEntityInterface $accessTokenEntity)
    {
        ...
        // Commit all deferred cache items to persistent storage
        try {
            $this->pool->commit();
            $this->logger->debug('Successfully committed access token to cache for token ' . $accessTokenEntity->getIdentifier());
        } catch (\Exception $e) {
            $this->logger->error('Failed to commit access token to cache: ' . $e->getMessage());
        }
    }
