...
class ApiAuthentication implements Middleware
{
    ...
    public function process(Request $request, RequestHandler $handler): Response
    {
        ...
        // Ensure ConfigService has dependencies set before trying to use it
        $configService = $container->get('configService');
        try {
            // Try to get a setting to see if dependencies are initialized
            $configService->getSetting('SERVER_KEY');
        } catch (\RuntimeException $e) {
            // If ConfigService hasn't had setDependencies called yet, call it now
            if (strpos($e->getMessage(), 'setDependencies') !== false) {
        ...
        $container->set('server', function(ContainerInterface $container) {
            ...
            try {
                ...
                $logger->error('LogicException when creating AuthorizationServer: ' . $exception->getMessage() . ' | Trace: ' . $exception->getTraceAsString());
